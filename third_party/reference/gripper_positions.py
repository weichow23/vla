...
def visualize_results(images, pr_pos, masks, bboxes):
    """
    Visualizes the corrected positions, transparent masks, and bounding boxes.
    """
    overlay_alpha = 0.5  # Transparency factor for the masks

    for i, (img, pos, mask, bbox) in enumerate(zip(images, pr_pos, masks, bboxes)):
        # Convert grayscale mask to a colored RGBA mask
        mask_colored = np.zeros_like(img, dtype=np.uint8)
        mask_colored[..., 1] = (mask * 255).astype(np.uint8)  # Green channel

        # Overlay mask with transparency
        img = cv2.addWeighted(img, 1, mask_colored, overlay_alpha, 0)

        # Draw bounding box
        xmin, ymin, xmax, ymax = bbox["box"]["xmin"], bbox["box"]["ymin"], bbox["box"]["xmax"], bbox["box"]["ymax"]
        cv2.rectangle(img, (xmin, ymin), (xmax, ymax), (0, 255, 255), 2)  # Yellow bounding box

        # Draw corrected 2D position
        cv2.circle(img, (int(pos[0]), int(pos[1])), radius=5, color=(255, 0, 0), thickness=-1)  # Red point

        images[i] = img

    # Save and show video
    mediapy.write_video("demo.mp4", images, fps=10)
...
def get_corrected_positions(episode_id, builder, plot=False):
    ...
    mask = [tr[1] for tr in t] # 256x256
    bbox = [tr[2] for tr in t] # dict {'score', 'label', 'box':{xmin, ymin, xmax, ymax}}
    ...
    if plot:
        visualize_results(images, pr_pos, mask, bbox)
        # images = [
...
pr_pos = get_corrected_positions(0, b_tfds, plot=True)