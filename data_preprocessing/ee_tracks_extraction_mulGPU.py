...
def get_gripper_masks(model, classifier, batched_images, batch_size, threshold=0.5):
    ...
    for i in range(0, len(batched_images), chunck_size):
        ...
        with torch.no_grad():
            ...
            classifier_output = classifier(dinov2_feats).squeeze() # bs, 197, 197
...
def main(args):
    ...
    for idx, episode in tqdm(enumerate(episodes_metadata.keys())):
        start= time.time()
        ...
        print(f"Episode: {episode}, Length: {len(images)}, Time: {time.time() - start}")
    ...
    print(f"Avg time: {total_time/10}")
